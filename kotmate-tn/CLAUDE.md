...
### Tablet
- Category rail moves from the left column to a horizontal strip across the **top** — a side rail leaves too little width once the cart panel is also on screen at tablet portrait sizes. Items and the running-order cart still sit side-by-side below it, though: this is **not** the simplified mobile flow — no bottom-sheet cart, no FAB, and table/waiter selection stays visible alongside the order at all times, only touch target sizing and panel widths scale down from desktop. Table/section and waiter selectors stay one tap away without opening a modal, since a tablet is often used standing/moving (e.g. a floor supervisor billing at a guest's table) rather than seated at a fixed counter. The persistent inline item-code field (useful when a Bluetooth numeric/barcode scanner is paired, common in TN retail, but not assumed) only has room from **1024px** wide (`lg`) — below that, including typical tablet-portrait widths, item-code entry is reached the same way it is on mobile: a header menu entry opens a touch keypad (see Mobile below). Widened from the original 768px (`md`) cutoff after production feedback found the inline field getting clipped out of a header that also has to fit search + KOT Tickets/Recall/Dashboard at in-between (phone-landscape/tablet-portrait) widths.
...
- **Item-code entry via an on-screen keypad, never a text field.** Below `lg` (1024px — every phone, and tablet-portrait too, see Tablet above) the header's "⋮" menu opens a modal with a big numeric display and a dedicated 1-9/0/clear/backspace keypad instead of a real `<input>` — a text input here would pop the Android/iOS on-screen keyboard over roughly half the modal on every open, fighting the popup for space and slower to hit than large on-screen keys (production feedback). Stays open after each add so a run of repeat-order codes can be entered back-to-back.
...
- **Repeat-KOT for add-on items** — ordering more of an item that's already been sent to the kitchen (e.g. a table asks for another round of Filter Coffee after the first ticket fired) never merges into that already-sent `order_items` row's quantity; it always becomes a new, still-unsent line for the same item (`order_items.line_no` places it after everything already in the cart, §8) — merging would leave the add-on permanently invisible to the next "Add to KOT" tap, since a repeat send only fires rows where `is_kot_sent` is still false. The POS cart shows both lines for that item, the already-fired one tagged **Sent**, so a cashier can see at a glance which portion the kitchen already has (production feedback). This split is a KOT/cart-screen concern only — billing consolidates back to one line per item (same `item_id` + `unit_price`) with the combined quantity, on the bill preview, the finalized bill response, and the actual print/reprint, since a customer's bill has no reason to show "Filter Coffee" twice just because it went to the kitchen in two rounds; `bill_items` itself still persists one row per originating `order_items` row underneath, for audit/reporting granularity (production feedback).
...
- **Monthly Sales Trend chart** — a bar chart on the tenant Dashboard, `tenant_admin`-only (enforced server-side, `GET /dashboard/sales-trend`, not just hidden in the UI, same convention as waiter billing §5/§9), available on every plan tier alongside the rest of the basic Dashboard KPIs. A Monthly/Yearly toggle switches the window: **Monthly** = a rolling 30-day window bucketed by day (not the current calendar month); **Yearly** = a rolling 12-month window bucketed by calendar month (not the current calendar year) — both trailing windows ending today/this month, IST-bucketed the same way the existing Hourly Sales Trend is (production feedback).
