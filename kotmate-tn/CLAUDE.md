...
- **Report printing** (Pro Max only, `plans.features.report_printing` + a tenant-wide `tenants.report_printing_enabled` soft-disable switch in Settings → Preferences, same shape as the stock-management switch) — any report on the Reports page can be sent to a `target="reports"` printer as plain ESC/POS-safe text (not the PDF/Excel/CSV export, which a thermal/dot-matrix printer can't render). Column widths are computed from the printer's actual `paper_width_mm`, not hardcoded: each column sizes to its real content width when it fits, and on a narrow 58mm printer where it doesn't, earlier (mostly text) columns shrink down to a floor before the last column (almost always the primary money figure — revenue, net sale value, incentive amount) does, so a currency total is never the one that gets cut off. Any `*_id` UUID column (e.g. `pos_user_id`) is dropped from the printed grid entirely — it's already duplicated by a human-readable `login_id`/`name` column on the same row and would otherwise just steal width as an unreadable truncated UUID.
