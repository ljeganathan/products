...
def dispatch_bill_print(printer: Printer, bill: BillRenderData) -> bytes:
    ...
    (bill_service._dispatch_print_for_bill): `usb`/`local_agent` printers are physically
    attached to the counter machine's browser, not this container, so those bytes go
    back to the frontend over WebUSB/local-agent instead; `network`/`wifi` printers are
    reachable directly from here via `printing.network_transport`. Bluetooth transport
