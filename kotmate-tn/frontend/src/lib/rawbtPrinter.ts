...
// (WiFi/network printers) or BLE-only support (Web Bluetooth, lib/webBluetoothPrinter.ts).
// This is the most robust option for a WiFi thermal printer reached from a cloud-hosted
// tenant, since the print job never has to leave the tablet's own LAN.
//
...
// pair or configure on KOTMate's side (CLAUDE.md §10); this transport just hands off
// bytes every time.
//
...
// limitation the underlying platform has, so the cashier-facing copy in
// PrintersPage.tsx tells them to confirm on the printer itself.
