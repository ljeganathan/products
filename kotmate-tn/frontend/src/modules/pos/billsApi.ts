...
// Raw ESC/POS bytes for a "usb"/"local_agent"/"bluetooth"/"rawbt" bill printer — the
// backend can't reach that printer itself (it's on the counter machine, not the
// server), so it hands these bytes back here for the frontend to forward via
// WebUSB/local-agent/Web Bluetooth/RawBT (lib/printDispatch.ts).
